...
export function HomePage({ lang }: { lang: Lang }) {
  ...
  return <SiteShell lang={lang}><main>
    <section className="shell hero"><div><p className="eyebrow">{t.intro}</p><h1>{t.hero}</h1><p className="hero-lead">{t.lead}</p><div className="hero-actions"><a className="button primary" href={`/${lang}/research`}>{t.viewResearch} →</a><a className="button" href={`/${lang}/blog`}>{t.readBlog}</a></div></div>
      <aside className="portrait-panel" aria-label="Profile portrait"><div className="portrait-frame"><Image src="/qianchen-guo.jpeg" width={1200} height={1600} priority alt={lang === 'zh' ? '郭乾琛肖像' : 'Portrait of Qianchen Guo'} /><span className="portrait-figure">FIG. 01 / PORTRAIT</span><span className="portrait-seal" aria-hidden="true">乾</span></div><div className="portrait-code"><b>● ACTIVE</b><br />{t.status}<br />{t.role}</div></aside>
    </section>
