...
export function BlogPage({ lang }: { lang: Lang }) {
  ...
  return <SiteShell lang={lang}><main><header className="page-hero"><div className="shell"><p className="eyebrow">04 / NOTES</p><h1>{cn ? '技术博客' : 'Technical Notes'}</h1><p>{cn ? '围绕控制、优化、决策规划与机器人学的长期技术写作。正文使用 Markdown 管理，原生支持 LaTeX 数学公式。' : 'Long-form notes on control, optimization, decision making, motion planning, and robotics. Posts are maintained in Markdown with native LaTeX support.'}</p></div></header>
    <section className="shell section"><div className="blog-toolbar"><span>{cn ? `${posts.length} 篇本地内容样例 · 知乎公开档案共 92 篇文章 / 78 个回答` : `${posts.length} locally curated sample(s) · 92 articles / 78 answers in the public Zhihu archive`}</span><span>MARKDOWN + KATEX</span></div>
      <div className="blog-list">{posts.map(post => <a className="post-card" key={post.slug} href={`/${lang}/blog/${post.slug}`}><div className="post-date">{post.date}</div><div><h2>{post.title}</h2><p>{post.excerpt}</p><div className="tag-list">{post.tags.map(tag => <span className="tag" key={tag}>{tag}</span>)}</div><span className="status">{cn ? '结构化整理版' : 'Editorial adaptation'}</span></div><span className="post-arrow">↗</span></a>)}</div>
